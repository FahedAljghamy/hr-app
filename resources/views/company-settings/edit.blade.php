{{-- 
Author: Eng.Fahed
Company Settings Edit View - HR System
صفحة تعديل إعدادات الشركة
--}}

@extends('layouts.app')

@section('title', trans('messages.Edit Settings'))

@section('content')
<!-- Page Heading -->
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <div>
        <h1 class="h3 mb-0 text-gray-800">{{ trans('messages.Edit Settings') }}</h1>
        <p class="text-muted">{{ trans('messages.Update your company information and preferences') }}</p>
    </div>
    <div>
        <a href="{{ route('company-settings.show', $companySetting) }}" class="d-none d-sm-inline-block btn btn-sm btn-secondary shadow-sm mr-2">
            <i class="fas fa-eye fa-sm text-white-50"></i> {{ trans('messages.View Settings') }}
        </a>
        <a href="{{ route('company-settings.index') }}" class="d-none d-sm-inline-block btn btn-sm btn-info shadow-sm">
...
<!-- Edit Company Settings Form -->
<form action="{{ route('company-settings.update', $companySetting) }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PUT')
                    ...
                    <div class="row">
                        ...
                        <div class="col-12 mb-3">
                            ...
                            <textarea name="address" id="address" rows="3" 
                                      ...
                                      placeholder="{{ trans('messages.Enter complete address') }}" required>{{ old('address', $companySetting->address) }}</textarea>
                            @error('address')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-12 mb-3">
                            ...
                            <textarea name="description" id="description" rows="4" 
                                      ...
                                      placeholder="{{ trans('messages.Enter company description') }}">{{ old('description', $companySetting->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
            ...
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    ...
                    <div class="text-center mb-3">
                        ...
                        <p class="text-muted small mt-2">{{ trans('messages.Current Logo') }}</p>
                    </div>
                    @endif
                    
                    <div class="form-group">
                        <label for="logo" class="form-label">{{ trans('messages.Upload New Logo') }}</label>
                        <input type="file" name="logo" id="logo" 
                               class="form-control-file @error('logo') is-invalid @enderror" 
                               accept="image/*">
                        <small class="form-text text-muted">{{ trans('messages.Leave empty to keep current logo') }}</small>
                        @error('logo')
                ...
                <div class="card-body">
                    <div class="form-group">
                        ...
                        <select name="timezone" id="timezone" class="form-control @error('timezone') is-invalid @enderror" required>
                            <option value="Asia/Damascus" {{ old('timezone', $companySetting->timezone) === 'Asia/Damascus' ? 'selected' : '' }}>{{ trans('messages.Damascus Time') }} (GMT+3)</option>
                            <option value="UTC" {{ old('timezone', $companySetting->timezone) === 'UTC' ? 'selected' : '' }}>UTC (GMT+0)</option>
                            <option value="Europe/London" {{ old('timezone', $companySetting->timezone) === 'Europe/London' ? 'selected' : '' }}>{{ trans('messages.London Time') }} (GMT+1)</option>
                        </select>
                        @error('timezone')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="form-group">
                        ...
                        <select name="currency" id="currency" class="form-control @error('currency') is-invalid @enderror" required>
                            <option value="SYP" {{ old('currency', $companySetting->currency) === 'SYP' ? 'selected' : '' }}>{{ trans('messages.Syrian Pound') }} (SYP)</option>
                            <option value="USD" {{ old('currency', $companySetting->currency) === 'USD' ? 'selected' : '' }}>{{ trans('messages.US Dollar') }} (USD)</option>
                            <option value="EUR" {{ old('currency', $companySetting->currency) === 'EUR' ? 'selected' : '' }}>{{ trans('messages.Euro') }} (EUR)</option>
                        </select>
                        @error('currency')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        ...
                        <div class="row">
                            <div class="col-6">
                                <input type="time" name="working_hours_start" 
                                       ...
                                       value="{{ old('working_hours_start', $companySetting->official_working_hours['start'] ?? '09:00') }}">
                            </div>
                            <div class="col-6">
                                <input type="time" name="working_hours_end" 
                                       class="form-control @error('working_hours_end') is-invalid @enderror" 
                                       value="{{ old('working_hours_end', $companySetting->official_working_hours['end'] ?? '17:00') }}">
                            </div>
                        </div>
            ...
            <div class="card shadow mb-4">
                <div class="card-header py-3">
                    <h6 class="m-0 font-weight-bold text-warning">
                        <i class="fas fa-share-alt mr-2"></i>{{ trans('messages.Social Media Links') }}
                    </h6>
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-3 mb-3">
                            ...
                            <input type="url" name="facebook" id="facebook" 
                                   value="{{ old('facebook', $companySetting->social_media['facebook'] ?? '') }}" 
                        ...
                        <div class="col-md-3 mb-3">
                            ...
                            <input type="url" name="twitter" id="twitter" 
                                   value="{{ old('twitter', $companySetting->social_media['twitter'] ?? '') }}" 
                        ...
                        <div class="col-md-3 mb-3">
                            ...
                            <input type="url" name="linkedin" id="linkedin" 
                                   value="{{ old('linkedin', $companySetting->social_media['linkedin'] ?? '') }}" 
                        ...
                        <div class="col-md-3 mb-3">
                            ...
                            <input type="url" name="instagram" id="instagram" 
                                   value="{{ old('instagram', $companySetting->social_media['instagram'] ?? '') }}" 
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="tax_number" id="tax_number" 
                                   value="{{ old('tax_number', $companySetting->tax_number) }}" 
            ...
            <div class="card shadow">
                <div class="card-body text-center">
                    <button type="submit" class="btn btn-primary btn-lg mr-3">
                        <i class="fas fa-save mr-2"></i>{{ trans('messages.Update Settings') }}
                    </button>
